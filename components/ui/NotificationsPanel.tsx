...
const NotificationsPanel: React.FC<NotificationsPanelProps> = ({ isOpen, onClose, notifications }) => {
    const { setCurrentView, setClientToView, setCaseToView, clients, cases } = useApp();
    ...
    console.log("🔔 [NotificationsPanel] Received:", notifications);
    ...
    // Catch-all for any other urgency or missing urgency
    const otherNotifications = notifications.filter(n => !['today', 'tomorrow', 'upcoming'].includes(n.urgency));
    ...
    const sections = [
        ...
        { id: 'others', label: 'Recentes / Outros', items: otherNotifications, color: 'slate' }
                        ...
                        {sections.map(section => {
                            ...
                            return (
                                <div key={section.id} className="space-y-2">
                                    ...
                                    >
                                        <h4 className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${section.color === 'red' ? 'text-red-400' :
                                            section.color === 'yellow' ? 'text-yellow-500' :
                                                section.color === 'gold' ? 'text-gold-500/70' : 'text-slate-400'
                                            }`}>
