...
const ClientCredentialsTab: React.FC<ClientCredentialsTabProps> = ({
    ...
    React.useEffect(() => {
        ...
        const loadPasswords = async () => {
            if (activeTab === 'credentials') {
                ...
                if (mounted) {
                    setDecryptedPasswords({ gov, inss });

                    // Se entrarmos em modo de edição, precisamos garantir que o editedClient
                    // tenha a senha descriptografada, senão o input mostrará o hash
                    if (isEditMode) {
                        setEditedClient({
                            ...editedClient,
                            senha_gov: editedClient.senha_gov === client.senha_gov ? gov : editedClient.senha_gov,
                            senha_inss: editedClient.senha_inss === client.senha_inss ? inss : editedClient.senha_inss
                        });
                    }
                }
            }
    ...
    }, [client.senha_gov, client.senha_inss, activeTab, isEditMode]); // Adicionado isEditMode para atualizar ao entrar em edição
                    ...
                    <div>
                        ...
                        {isEditMode ? (
                            ...
                            <div className="bg-[#131418] border border-white/5 rounded-lg px-4 py-3 text-white font-mono flex justify-between items-center group">
                                <span>
                                    {client.senha_gov
                                        ? (showGovPassword ? decryptedPasswords.gov : '••••••••••••')
                    ...
                    <div>
                        ...
                        {isEditMode ? (
                            ...
                            <div className="bg-[#131418] border border-white/5 rounded-lg px-4 py-3 text-white font-mono flex justify-between items-center group">
                                <span>
                                    {client.senha_inss
                                        ? (showInssPassword ? decryptedPasswords.inss : '••••••••••••')
