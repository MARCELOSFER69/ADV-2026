...
interface CaseDocsTabProps {
    ...
    activeTab: string;
    isUploading: boolean;
    handleDocumentUpload: (e: React.ChangeEvent<HTMLInputElement>, caseId?: string) => void;
            ...
            <div className="bg-[#18181b] border-2 border-dashed border-white/5 rounded-xl p-8 flex flex-col items-center justify-center text-center transition-colors hover:border-gold-500/50 hover:bg-[#18181b]/80 group">
                {isUploading ? (
                    ...
                    <label className="cursor-pointer w-full flex flex-col items-center">
                        ...
                        <h3 className="text-sm font-bold text-white mb-1">Clique para anexar ao processo</h3>
                        <p className="text-xs text-zinc-500">PDF ou Imagens (JPG, PNG)</p>
                        <input
                            type="file"
                            className="hidden"
                            onChange={(e) => handleDocumentUpload(e, caseId)}
            ...
            <div className="space-y-3">
                {caseDocuments.length > 0 ? (
                    ...
                    <div className="text-center py-8 bg-navy-900/30 rounded-xl border border-dashed border-zinc-800">
                        <p className="text-xs text-zinc-500 italic">Nenhum documento anexado a este processo.</p>
                    </div>
                )}
            </div>
        </div>
...
export default memo(CaseDocsTab);
