...
const ClientInfoTab: React.FC<ClientInfoTabProps> = ({
    ...
    return (
        <>
            ...
            {!isEditMode && (
                <div className="mb-6 bg-navy-900 border border-slate-800 rounded-xl p-4 shadow-sm">
                    <h4 className="text-sm font-bold text-slate-400 uppercase mb-3 flex items-center gap-2"><Users size={16} /> Entrevista Inicial</h4>
            ...
            {isEditMode && (
                <div className="mb-6 p-4 bg-navy-900 border border-slate-800 rounded-xl">
                    <h5 className="text-sm font-bold text-gold-500 mb-3 flex items-center gap-2"><Users size={16} /> Gestão de Entrevista</h5>
                    <div className="flex flex-wrap gap-4">
                        <div className="flex-1 min-w-[200px]">
                            <label className="block text-xs font-bold text-slate-500 mb-1">Status da Entrevista</label>
                            <select
                                className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1.5 text-sm text-white outline-none focus:border-gold-500"
                                value={editedClient.interviewStatus || 'Pendente'}
                                onChange={(e) => setEditedClient({ ...editedClient, interviewStatus: e.target.value as any })}
                            >
                        ...
                        {editedClient.interviewStatus === 'Agendada' && (
                            <div className="flex-1 min-w-[200px] animate-in slide-in-from-left-2">
                                ...
                                <input
                                    type="date"
                                    className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1.5 text-sm text-white outline-none focus:border-gold-500 [color-scheme:dark]"
            ...
            {isEditMode ? (
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Data Nascimento</label><input type="date" className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white [color-scheme:dark]" value={editedClient.data_nascimento || ''} onChange={e => setEditedClient({ ...editedClient, data_nascimento: e.target.value })} /></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Sexo</label><select className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.sexo || ''} onChange={e => setEditedClient({ ...editedClient, sexo: e.target.value as any })}><option value="">Selecione</option><option value="Masculino">Masculino</option><option value="Feminino">Feminino</option></select></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Telefone</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.telefone || ''} onChange={e => setEditedClient({ ...editedClient, telefone: formatPhone(e.target.value) })} /></div>
                    <div className="md:col-span-2"><label className="block text-xs font-bold text-slate-500 mb-1">Email</label><input type="email" className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.email || ''} onChange={e => setEditedClient({ ...editedClient, email: e.target.value })} /></div>

                    <div className="md:col-span-3 border-t border-slate-800 pt-4 mt-2 mb-2">
                        ...
                        {hasRepresentative && (
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 animate-in slide-in-from-top-2 fade-in duration-200 bg-zinc-900/50 p-4 rounded-xl border border-zinc-800">
                                <div>
                                    <label className="block text-xs font-bold text-zinc-400 uppercase mb-1">Nome do Representante</label>
                                    <div className="relative group">
                                        <User className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-yellow-600 transition-colors" size={16} />
                                        <input
                                            className="w-full bg-[#0f1014] border border-zinc-800 text-zinc-200 pl-9 pr-4 py-2 rounded-lg focus:border-yellow-600 focus:ring-1 focus:ring-yellow-600/20 outline-none placeholder:text-zinc-600"
                                ...
                                <div>
                                    ...
                                    <div className="relative group">
                                        <CreditCard className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-600 group-focus-within:text-yellow-600 transition-colors" size={16} />
                                        <input
                                            className="w-full bg-[#0f1014] border border-zinc-800 text-zinc-200 pl-9 pr-4 py-2 rounded-lg focus:border-yellow-600 focus:ring-1 focus:ring-yellow-600/20 outline-none placeholder:text-zinc-600"
                                            value={editedClient.representante_cpf || ''}
                                            onChange={e => setEditedClient({ ...editedClient, representante_cpf: e.target.value })} // Assumes formatter not strictly enforced here or use simple input
                    ...
                    <div className="md:col-span-3 border-t border-slate-800 pt-4 mt-2"><h5 className="text-sm font-bold text-gold-500 mb-3 flex items-center gap-2"><Building2 size={16} /> Sistema & Origem</h5></div>
                    ...
                    <div className="md:col-span-2 relative"><label className="block text-xs font-bold text-slate-500 mb-1">Captador {editedClient.filial && <span className="text-xs font-normal text-slate-600 ml-1">({filteredCaptadores.length} disponíveis)</span>}</label><div className="flex gap-2">{isAddingCaptador ? (<div className="flex-1 flex gap-2 animate-in slide-in-from-left-2 fade-in duration-200"><input autoFocus className="w-full bg-navy-800 border border-slate-700 rounded-xl px-3 py-2 text-sm text-white outline-none focus:border-gold-500" placeholder="Nome do novo captador..." value={newCaptadorName} onChange={(e) => setNewCaptadorName(e.target.value)} onKeyDown={(e) => { if (e.key === 'Enter') handleAddCaptador(); if (e.key === 'Escape') { setIsAddingCaptador(false); setNewCaptadorName(''); } }} /><button onClick={handleAddCaptador} className="bg-emerald-600 hover:bg-emerald-500 text-white p-2 rounded-lg transition-colors"><Check size={18} /></button><button onClick={() => { setIsAddingCaptador(false); setNewCaptadorName(''); }} className="bg-red-600 hover:bg-red-500 text-white p-2 rounded-lg transition-colors"><X size={18} /></button></div>) : (<div className="flex-1 flex gap-2"><div className="flex-1"><CustomSelect label="" value={editedClient.captador || ''} onChange={(val) => setEditedClient({ ...editedClient, captador: val })} options={filteredCaptadores.map(c => ({ label: c.nome, value: c.nome }))} icon={Share2} placeholder={editedClient.filial ? "Selecione..." : "Selecione uma filial primeiro"} /></div>{editedClient.filial && (<button onClick={() => setIsAddingCaptador(true)} className="bg-navy-800 border border-slate-700 hover:border-gold-500 hover:text-gold-500 text-slate-400 p-2.5 rounded-xl transition-all h-[42px] mt-auto"><Plus size={18} /></button>)}{editedClient.filial && editedClient.captador && (<button onClick={handleDeleteCaptadorInit} className="bg-navy-800 border border-slate-700 hover:border-red-500 hover:text-red-500 text-slate-400 p-2.5 rounded-xl transition-all h-[42px] mt-auto"><Trash2 size={18} /></button>)}</div>)}</div></div>
                    <div className="md:col-span-3 border-t border-slate-800 pt-4 mt-2"><h5 className="text-sm font-bold text-gold-500 mb-3">Documentação</h5></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">RG</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.rg || ''} onChange={e => setEditedClient({ ...editedClient, rg: e.target.value })} /></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Org. Emissor</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.orgao_emissor || ''} onChange={e => setEditedClient({ ...editedClient, orgao_emissor: e.target.value })} /></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Profissão</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.profissao || ''} onChange={e => setEditedClient({ ...editedClient, profissao: e.target.value })} /></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Estado Civil</label><select className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.estado_civil || ''} onChange={e => setEditedClient({ ...editedClient, estado_civil: e.target.value })}><option value="">Selecione</option><option value="Solteiro(a)">Solteiro(a)</option><option value="Casado(a)">Casado(a)</option><option value="Divorciado(a)">Divorciado(a)</option><option value="Viúvo(a)">Viúvo(a)</option><option value="União Estável">União Estável</option></select></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Nacionalidade</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.nacionalidade || ''} onChange={e => setEditedClient({ ...editedClient, nacionalidade: e.target.value })} /></div>
                    <div className="md:col-span-3 border-t border-slate-800 pt-4 mt-2"><h5 className="text-sm font-bold text-gold-500 mb-3">Endereço</h5></div>
                    <div className="relative"><label className="block text-xs font-bold text-slate-500 mb-1">CEP</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.cep || ''} onChange={handleCepChange} /></div>
                    <div className="md:col-span-2"><label className="block text-xs font-bold text-slate-500 mb-1">Logradouro</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.endereco || ''} onChange={e => setEditedClient({ ...editedClient, endereco: e.target.value })} /></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Número</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.numero_casa || ''} onChange={e => setEditedClient({ ...editedClient, numero_casa: e.target.value })} /></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Bairro</label><input className="w-full bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.bairro || ''} onChange={e => setEditedClient({ ...editedClient, bairro: e.target.value })} /></div>
                    <div><label className="block text-xs font-bold text-slate-500 mb-1">Cidade/UF</label><div className="flex gap-2"><input className="w-2/3 bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.cidade || ''} onChange={e => setEditedClient({ ...editedClient, cidade: e.target.value })} /><input className="w-1/3 bg-navy-800 border border-slate-700 rounded px-2 py-1 text-sm text-white" value={editedClient.uf || ''} onChange={e => setEditedClient({ ...editedClient, uf: e.target.value })} /></div></div>
                    <div className="md:col-span-3 border-t border-slate-800 pt-4 mt-2"><h5 className="text-sm font-bold text-red-400 mb-3">Pendências</h5></div>
                    <div className="md:col-span-3 flex flex-wrap gap-2">{PENDING_OPTIONS.map(option => { const isSelected = editedClient.pendencias?.includes(option); return (<button key={option} type="button" onClick={() => togglePendencia(option)} className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${isSelected ? 'bg-red-500/20 border-red-500 text-red-400' : 'bg-zinc-800 border-zinc-700 text-slate-400 hover:text-white'}`}>{option}</button>); })}</div>
                    <div className="md:col-span-3 border-t border-slate-800 pt-4 mt-2">
                        <label className="block text-xs font-bold text-slate-500 mb-1">Observação</label>
                        <textarea
                            className="w-full bg-navy-800 border border-slate-700 rounded-lg p-3 text-sm text-white outline-none focus:border-gold-500 resize-none h-24"
            ...
            {!isEditMode && retirementInfo && (
                <div className="bg-navy-900 border border-slate-800 rounded-xl p-4 mb-6">
                    <h4 className="text-sm font-bold text-slate-400 uppercase mb-3 flex items-center gap-2"><Clock size={16} /> Previsão de Aposentadoria</h4>
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div className="bg-navy-950 p-3 rounded-lg border border-slate-800 text-center"><span className="block text-xs text-slate-500 uppercase mb-1">Idade Atual</span><span className="text-xl font-bold text-white">{retirementInfo.age.years} <span className="text-xs font-normal text-slate-400">anos</span></span></div>
                        <div className="bg-navy-950 p-3 rounded-lg border border-slate-800 text-center relative overflow-hidden"><span className="block text-xs text-slate-500 uppercase mb-1">Rural ({retirementInfo.rural.target} anos)</span>{retirementInfo.rural.eligible ? <span className="text-emerald-500 font-bold text-lg animate-pulse">JÁ ELEGÍVEL</span> : <span className="text-white font-bold text-lg">{Math.floor(retirementInfo.rural.remaining)} <span className="text-xs font-normal text-slate-400">anos falta</span></span>}{retirementInfo.rural.eligible && <div className="absolute inset-0 border-2 border-emerald-500/50 rounded-lg"></div>}</div>
                        <div className="bg-navy-950 p-3 rounded-lg border border-slate-800 text-center"><span className="block text-xs text-slate-500 uppercase mb-1">Urbana ({retirementInfo.urban.target} anos)</span>{retirementInfo.urban.eligible ? <span className="text-emerald-500 font-bold text-lg">JÁ ELEGÍVEL</span> : <span className="text-white font-bold text-lg">{Math.floor(retirementInfo.urban.remaining)} <span className="text-xs font-normal text-slate-400">anos falta</span></span>}</div>
                    </div>
                </div>
            )}
            ...
            {!isEditMode && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
                    <div className="p-4 bg-navy-900 border border-slate-800 rounded-lg"><span className="text-xs text-slate-500 uppercase font-bold block mb-1">Telefone</span><div className="flex items-center gap-2"><span className="text-slate-200 font-medium">{String(client.telefone || '-')}</span>{client.telefone && <button onClick={() => setIsWhatsAppModalOpen(true)} className="text-emerald-500 hover:text-emerald-400 p-1 rounded hover:bg-emerald-500/10 transition-colors" title="WhatsApp"><MessageCircle size={16} /></button>}</div></div>
                    <div className="p-4 bg-navy-900 border border-slate-800 rounded-lg"><span className="text-xs text-slate-500 uppercase font-bold block mb-1">Email</span><span className="text-slate-200 font-medium truncate block" title={client.email}>{String(client.email || '-')}</span></div>
                    <div className="p-4 bg-navy-900 border border-slate-800 rounded-lg md:col-span-2"><span className="text-xs text-slate-500 uppercase font-bold block mb-1">Endereço</span><span className="text-slate-200 font-medium flex items-center gap-2"><MapPin size={14} /> {String(client.endereco ? `${client.endereco}, ${client.numero_casa || 'S/N'} - ${client.bairro || ''}, ${client.cidade || ''} - ${client.uf || ''}` : '-')}</span></div>
                    <div className="p-4 bg-navy-900 border border-slate-800 rounded-lg"><span className="text-xs text-slate-500 uppercase font-bold block mb-1">Data Nascimento</span><span className="text-slate-200 font-medium">{formatDateDisplay(client.data_nascimento)}</span></div>
                    <div className="p-4 bg-navy-900 border border-slate-800 rounded-lg"><span className="text-xs text-slate-500 uppercase font-bold block mb-1">Filial</span><span className="text-emerald-500 font-bold">{String(client.filial || 'Matriz')}</span></div>
                    <div className="p-4 bg-navy-900 border border-slate-800 rounded-lg md:col-span-2"><span className="text-xs text-slate-500 uppercase font-bold block mb-1">Observação</span><p className="text-slate-300 text-sm italic">{String(client.observacao || 'Sem observações.')}</p></div>

                    {client.representante_nome && (
                        <div className="p-4 bg-navy-900 border border-slate-800 rounded-lg md:col-span-4 ring-1 ring-gold-600/20">
            ...
            <div className="flex items-center justify-between mb-4">
                ...
                {!isEditMode && (
                    <div className="relative">
                        ...
                        {showNewCaseMenu && (
                            <div className="absolute right-0 bottom-full mb-2 w-56 bg-navy-900 border border-slate-700 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100 flex flex-col ring-1 ring-black/50">
                                <div className="px-3 py-2 bg-zinc-950/50 border-b border-zinc-800 text-[10px] uppercase font-bold text-zinc-500">
                                    Tipo de Processo
                                </div>
                                <button onClick={() => { openNewCaseWithParams(client.id, 'Seguro Defeso' as any); setShowNewCaseMenu(false); onClose(); }} className="text-left px-4 py-3 text-sm text-slate-300 hover:bg-slate-800 hover:text-emerald-400 transition-colors border-b border-slate-800 flex items-center gap-2">
                                    <Shield size={16} /> Seguro Defeso
                                </button>
                                <button onClick={() => { openNewCaseWithParams(client.id, 'Aposentadoria' as any); setShowNewCaseMenu(false); onClose(); }} className="text-left px-4 py-3 text-sm text-slate-300 hover:bg-slate-800 hover:text-blue-400 transition-colors border-b border-slate-800 flex items-center gap-2">
                                    <FileText size={16} /> Administrativo
                                </button>
                                <button onClick={() => { openNewCaseWithParams(client.id, 'Cível/Outros' as any); setShowNewCaseMenu(false); onClose(); }} className="text-left px-4 py-3 text-sm text-slate-300 hover:bg-slate-800 hover:text-amber-400 transition-colors flex items-center gap-2">
            ...
            <div className="space-y-3 mb-8">
                {cases.filter(c => c.client_id === client.id).map(caseItem => (
                    <div key={caseItem.id} onClick={() => onSelectCase?.(caseItem)} className="flex items-center justify-between p-4 bg-navy-900 border border-slate-800 rounded-lg hover:border-gold-600/30 cursor-pointer group transition-colors">
                        <div><h4 className="font-bold text-slate-200 text-sm group-hover:text-gold-500 transition-colors">{caseItem.titulo}</h4><p className="text-xs text-slate-500">{caseItem.numero_processo} • {caseItem.tribunal}</p></div>
