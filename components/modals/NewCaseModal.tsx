...
const NewCaseModal: React.FC<NewCaseModalProps> = ({ isOpen, onClose, forcedType }) => {
    ...
    const { addCase, clients = [], cases = [], showToast, newCaseParams, user, saveUserPreferences } = useApp();
        ...
        if (isOpen) {
            ...
            if (newCaseParams) {
                ...
                if (newCaseParams.clientId) {
                    ...
                    if (c) initialClientName = c.nome_completo;
                }
    ...
    }, [isOpen, forcedType, newCaseParams, clients]);
    ...
    const filteredClients = useMemo(() => {
        const activeOnly = (clients || []).filter(c => c.status !== 'arquivado');
        const term = (clientSearchTerm || '').toLowerCase();

        const list = activeOnly.filter(c =>
            (c.nome_completo?.toLowerCase() || '').includes(term) ||
            (c.cpf_cnpj || '').includes(term)
        );

        return list.length > 0 ? list.sort((a, b) => (a.nome_completo || '').localeCompare(b.nome_completo || '')) : [];
                ...
                <form onSubmit={handleCreateCase} className="flex-1 overflow-y-auto custom-scrollbar p-6 space-y-6">
                    ...
                    <div className="relative" ref={searchWrapperRef}>
                        ...
                        <div className="relative">
                            ...
                            />
                            <div className="absolute right-3 top-1/2 -translate-y-1/2 text-zinc-500 pointer-events-none"><ChevronDown size={16} /></div>
                        </div>
                        {isClientListOpen && (
