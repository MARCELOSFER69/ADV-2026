    ...
    return (
        <div className="mb-6 space-y-4">
            <div className="bg-zinc-900/60 backdrop-blur-md border border-white/5 rounded-xl p-4 shadow-2xl relative z-20">
                ...
                {showSort && (
                    <div className="pt-4 border-t border-white/5 mt-4 animate-in slide-in-from-top-2">
                        <div className="flex flex-wrap gap-2">
                            <button
                                onClick={() => handleSort('nome_completo')}
                                className={`px-4 py-2 rounded-lg text-sm flex items-center gap-2 transition-all ${sortConfig.key === 'nome_completo' ? 'bg-gold-600 text-white shadow-lg shadow-gold-600/20' : 'bg-navy-950/50 text-slate-400 border border-white/10 hover:border-white/20'}`}
                            >
                                Nome {sortConfig.key === 'nome_completo' && (sortConfig.direction === 'asc' ? <ArrowDown size={14} /> : <ArrowUp size={14} />)}
                            </button>
                            <button
                                onClick={() => handleSort('filial')}
                                className={`px-4 py-2 rounded-lg text-sm flex items-center gap-2 transition-all ${sortConfig.key === 'filial' ? 'bg-gold-600 text-white shadow-lg shadow-gold-600/20' : 'bg-navy-950/50 text-slate-400 border border-white/10 hover:border-white/20'}`}
                            >
                                Filial {sortConfig.key === 'filial' && (sortConfig.direction === 'asc' ? <ArrowDown size={14} /> : <ArrowUp size={14} />)}
                            </button>
                            <button
                                onClick={() => handleSort('captador')}
                                className={`px-4 py-2 rounded-lg text-sm flex items-center gap-2 transition-all ${sortConfig.key === 'captador' ? 'bg-gold-600 text-white shadow-lg shadow-gold-600/20' : 'bg-navy-950/50 text-slate-400 border border-white/10 hover:border-white/20'}`}
                            >
                ...
                {showColumnConfig && (
                    <div className="pt-4 border-t border-white/5 mt-4 animate-in slide-in-from-top-2">
                        ...
                        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
                            {columns.map((col, idx) => (
                                <div key={col.id} className="flex items-center justify-between p-2 bg-navy-950/50 border border-white/5 rounded-lg group hover:border-white/10 transition-all">
                ...
                {showFilters && (
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-4 pt-4 border-t border-white/5 mt-4 animate-in slide-in-from-top-2">
                        <div>
                            <label className="block text-xs font-medium text-slate-500 mb-1">Filial</label>
                            <select
                                className="w-full bg-navy-950/50 text-white px-3 py-2 border border-white/10 rounded-lg text-sm outline-none focus:border-gold-500"
                        ...
                        <div className="md:col-span-2">
                            ...
                            <div className="flex gap-2">
                                <input
                                    type="date"
                                    className="w-full bg-navy-950/50 text-white px-3 py-2 border border-white/10 rounded-lg text-sm outline-none [color-scheme:dark]"
