        ...
        >
            ...
            {columns.filter(c => c.visible).map(col => (
                <td key={`${client.id}-${col.id}`} className="px-6 py-4 align-middle">
                    ...
                    {col.id === 'casos_titulos' && (
                        <span className="text-xs text-slate-400 truncate max-w-[200px] block font-medium" title={client.casos_titulos as string}>
                            {String(client.casos_titulos || '-')}
                        </span>
                    )}
