...
import { useKanbanCases } from '../hooks/useCases'; // Alterado para buscar apenas dados leves
import { useAllClients } from '../hooks/useClients';
import { Case, CaseStatus } from '../types';
import { AlertCircle, CheckCircle2, Circle, Clock, FileText, X, ChevronDown, Search } from 'lucide-react';
...
// Colunas que queremos exibir no Kanban
...
const KanbanBoard: React.FC<KanbanBoardProps> = ({ onCaseClick, statusFilters }) => {
    const { updateCase, showToast } = useApp();

    // USANDO HOOK OTIMIZADO (Apenas colunas necessárias)
    const { data: cases = [], isLoading } = useKanbanCases();

    // Precisamos dos clientes para mostrar o nome? 
    // Se o backend já trouxer client_name no view, melhor. 
    // Mas o fetchKanbanCases traz client_id. Vamos manter o hook de clients por enquanto, 
    // mas idealmente o Kanban deve ter client_name no objeto case para evitar join no front.
    // Falha: O fetchKanbanCases não traz nome do cliente, e o hook useClients baixa tudo?
    // Correção Rápida: Manter uso de clients cacheado ou ajustar fetchKanbanCases para trazer nome.
    // Assumindo que useAllClients é cacheado, ok. Se for pesado, refatorar depois.
    ...
    // Estado Local para Paginação por Coluna
    ...
    const handleLoadMore = (status: string) => {
        setVisibleCounts(prev => ({
            ...prev,
            [status]: (prev[status] || ITEMS_PER_LOAD) + ITEMS_PER_LOAD
        }));
    };
    ...
    const columns = useMemo(() => {
        ...
        cases.forEach(c => {
            // Filtro de Texto Local (Rápido para UX)
            if (localSearch) {
                ...
                if (!title.includes(searchLower) && !clientName.includes(searchLower) && !c.numero_processo?.includes(searchLower)) {
                    return;
                }
            }
    ...
    if (isLoading && cases.length === 0) {
        return <div className="p-8 text-center text-zinc-500">Carregando Kanban Otimizado...</div>;
    }

    return (
        <div className="h-full flex flex-col">
            {/* Barra de Filtro Local do Kanban */}
            <div className="flex justify-between items-center mb-4 px-1">
                <div className="relative w-64">
                    <Search className="absolute left-2 top-2.5 text-zinc-500" size={16} />
                    <input
                        type="text"
                        placeholder="Filtrar no quadro..."
                        value={localSearch}
                        onChange={(e) => setLocalSearch(e.target.value)}
                        className="w-full bg-zinc-900 border border-zinc-700 rounded-lg pl-8 p-2 text-sm text-zinc-200 focus:ring-1 focus:ring-yellow-600 outline-none"
                    />
                </div>
                <div className="text-xs text-zinc-500">
                    Mostrando {cases.length} processos
                </div>
            </div>
            ...
            <div className="flex-1 overflow-x-auto pb-4">
                <div className="flex gap-4 min-w-[1200px] h-full">
                    {KANBAN_COLUMNS.map(status => {
                        ...
                        const limit = visibleCounts[status] || ITEMS_PER_LOAD;
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className={`p-3 border-b border-zinc-800 flex justify-between items-center ${config.bg} rounded-t-xl`}>
                                    ...
                                    <span className="bg-black/30 text-white text-[10px] px-2 py-0.5 rounded-full font-bold">{allItems.length}</span>
                                </div>

                                {/* Área de Cards com Virtualização Manual */}
                                <div className="flex-1 p-2 overflow-y-auto custom-scrollbar space-y-2">
                                    {visibleItems.map(c => {
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <p className="text-xs text-zinc-500 mb-2">{client?.nome_completo || '...'}</p>
                                    ...
                                    {hasMore && (
                                        <button
                                            ...
                                            className="w-full py-2 text-xs text-zinc-500 hover:text-zinc-300 hover:bg-zinc-800/50 rounded flex items-center justify-center gap-1 transition-colors"
                                        >
                                            <ChevronDown size={14} />
                                            Ver mais ({allItems.length - limit})
                                        </button>
                                    )}

                                    {allItems.length === 0 && (
                                        <div className="h-full flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity">
                                            <p className="text-[10px] text-zinc-600 uppercase font-bold border-2 border-dashed border-zinc-800 p-4 rounded-lg">Vazio</p>
                                        </div>
                                    )}
                                </div>
                            </div>
                ...
                <div className="fixed inset-0 bg-black/80 z-[9999] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in zoom-in duration-200">
                    <div className="bg-zinc-900 border border-zinc-700 p-6 rounded-xl max-w-sm w-full shadow-2xl">
                        ...
                        <p className="text-zinc-400 text-sm mb-6">
                            Deseja mover para <strong>{pendingMove.newStatus}</strong>?
                        </p>
