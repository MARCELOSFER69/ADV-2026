...
app.get('/api/stream-reap', async (req, res) => {
    ...
    try {
        ...
        const result = await runReapProcess(id, cpf, decryptedPassword, isHeadless, (logMessage) => {
            ...
            currentPid = pid;
            activePids.add(pid);
            console.log(`[RGP] Processo monitorado iniciado: PID ${pid}`);
        });

        if (currentPid) activePids.delete(currentPid);
...
app.post('/api/stop-reap', async (req, res) => {
    console.log('🛑 [API] Solicitação para parar robôs REAP recebida.');
    try {
        if (activePids.size === 0) {
            console.log('⚠️ Nenhum processo REAP ativo rastreado.');
            return res.json({ success: true, message: 'Nenhum processo ativo para parar.' });
        }

        const pids = Array.from(activePids);
        console.log(`🔪 Matando ${pids.length} processos: ${pids.join(', ')}`);

        let killedCount = 0;
        for (const pid of pids) {
            treeKill(pid, 'SIGKILL', (err) => {
                if (err) {
                    console.error(`❌ Erro ao matar PID ${pid}:`, err);
                } else {
        ...
        res.json({ success: true, message: `Comando enviado para encerrar ${pids.length} processos.` });
    } catch (error) {
