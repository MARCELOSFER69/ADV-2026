-- Versão Marcelo V3: Exclui processos ARQUIVADOS da agregação e do status calculado
...
SELECT 
    ...
    COALESCE((
        ...
        AND cs.status != 'Arquivado' -- EXCLUI PROCESSOS ARQUIVADOS DA COLUNA
    ), '') AS casos_titulos,
    COALESCE((
        SELECT unaccent(string_agg(cs.tipo || COALESCE(' (' || cs.modalidade || ')', ''), ', ')) 
        FROM cases cs 
        WHERE cs.client_id = c.id 
        AND cs.status != 'Arquivado' -- EXCLUI PROCESSOS ARQUIVADOS DA BUSCA
    ...
    CASE 
        ...
        WHEN EXISTS (
            SELECT 1 FROM cases cs 
            WHERE cs.client_id = c.id
            AND cs.status != 'Arquivado' -- SÓ CONTA COMO INDEFERIDO SE NÃO ESTIVER ARQUIVADO
        ) THEN 'Indeferido'
        ELSE 'Inativo'
