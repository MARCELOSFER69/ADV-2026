...
const DocumentBuilder: React.FC = () => {
  ...
  const renderField = (field: FieldMark) => {
      ...
      return (
          <div
              key={field.id}
              onMouseDown={(e) => handleMouseDown(e, field.id)}
              className={`field-mark absolute select-none ${
                  isPreviewMode ? '' : (isSelected ? 'border border-blue-500 bg-blue-500/10' : 'border border-dashed border-transparent hover:border-zinc-400 bg-yellow-500/20')
              }`}
              style={{
                  ...commonStyle,
  ...
  return (
    <div className="h-full flex flex-col space-y-4 pb-4 animate-in fade-in duration-300">
        <div className="flex justify-between items-center border-b border-zinc-800 pb-4">
            <h2 className="text-2xl font-bold text-white font-serif flex items-center gap-2">
                {baseType === 'pdf' ? <FileText className="text-gold-500" /> : <Code className="text-blue-500" />} 
                Editor de Modelos {baseType === 'html' && '(HTML)'}
            </h2>
                    ...
                    <>
                        {baseType === 'html' && !isPreviewMode && (
                            <button 
                                ...
                                className={`px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 ${isCodeMode ? 'bg-purple-600 text-white' : 'bg-zinc-800 text-zinc-300 hover:text-white'}`}
                            >
                                <FileCode size={16}/> {isCodeMode ? 'Visual' : 'Código'}
                            </button>
                        )}
                        <button onClick={() => setIsPreviewMode(!isPreviewMode)} className={`px-3 py-2 rounded-lg text-sm font-bold flex items-center gap-2 ${isPreviewMode ? 'bg-blue-600 text-white' : 'bg-zinc-800 text-zinc-300 hover:text-white'}`}>
                            {isPreviewMode ? <EyeOff size={16}/> : <Eye size={16}/>} Preview
                        </button>
                        <button onClick={handleSave} disabled={isSaving} className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded-lg font-bold flex items-center gap-2 shadow-lg">
