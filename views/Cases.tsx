...
const Cases: React.FC = () => {
    ...
    const { data: paginatedCases, isLoading: isFetching, totalCount: totalCases, refetch } = useCases(
        ...
        ITEMS_PER_PAGE,
        {
            ...
            quickFilter // Passando quickFilter para o servidor futuramente
        }
    );
    ...
    const displayedCases = useMemo(() => {
        ...
        // Apply quick filters locally if not yet supported by server
        if (quickFilter === 'mine') {
    ...
    const getClientName = (id: string, caseItem?: Case) => {
        if ((caseItem as any)?.client_name) return (caseItem as any).client_name;
    ...
    useEffect(() => {
        if (caseToView) {
            const c = displayedCases.find(c => c.id === caseToView);
            if (c) {
                setSelectedCase(c);
    ...
    }, [caseToView, displayedCases, setCaseToView]);
                    ...
                    >
                        <div className="flex gap-6 min-w-max h-full px-2">
                            {activeKanbanColumns.map((status) => {
                                const columnCases = displayedCases.filter(c => c.status === status);
