...
const Expertise: React.FC = () => {
    ...
    }, [events, searchTerm, filterMode, cases, clients]);
    ...
    const handleUpdateEventLocal = async () => {
        ...
        await updateEvent({
            ...editingEvent,
            tipo: isCustomEditEventType ? (customEventType as any) : editingEvent.tipo,
    ...
    const handleDeleteEvent = async (id: string) => {
        // Ação direta para diagnosticar se o confirm era o problema
        try {
        ...
        <div className="h-full flex flex-col p-6 animate-in fade-in duration-500">
            ...
            <div className="bg-[#0f1014] border border-zinc-800 rounded-xl p-4 mb-6 flex flex-col md:flex-row gap-4 items-center justify-between shadow-sm">
                <div className="relative w-full md:w-96">
                    ...
                    <input
                        className="w-full bg-[#09090b] border border-zinc-800 rounded-lg pl-10 pr-4 py-2.5 text-zinc-200 outline-none focus:border-gold-500 transition-colors"
                        placeholder="Buscar por cliente, cidade ou data..."
                        value={searchTerm}
                ...
                <div className="flex bg-[#09090b] p-1 rounded-lg border border-zinc-800">
                    {(['upcoming', 'past', 'all'] as const).map(mode => (
                        ...
                        >
                            {mode === 'upcoming' ? 'Próximas' : mode === 'past' ? 'Passadas' : 'Todas'}
                        </button>
                    ))}
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4 overflow-y-auto pb-20 custom-scrollbar">
                {pericias.length > 0 ? pericias.map(event => {
                    const client = getClient(event.case_id);
                    const eventDate = new Date(event.data_hora);
                    const isToday = new Date().toDateString() === eventDate.toDateString();

                    return (
                        <div key={event.id} className="bg-[#0f1014] border border-zinc-800/50 hover:border-gold-500/30 rounded-xl p-5 flex flex-col md:flex-row items-center gap-6 group transition-all hover:bg-zinc-900/30 relative overflow-hidden">
                            {isToday && <div className="absolute left-0 top-0 bottom-0 w-1 bg-gold-500 shadow-[0_0_10px_rgba(234,179,8,0.5)]" />}
                            ...
                            <div className="flex flex-col items-center justify-center min-w-[80px] text-center">
                                ...
                                <span className={`text-2xl font-bold ${isToday ? 'text-gold-500' : 'text-white'}`}>{eventDate.getDate()}</span>
                            ...
                            <div className="flex-1 w-full">
                                {editingEvent?.id === event.id ? (
                                    <div className="bg-zinc-900/50 p-4 rounded-xl border border-gold-500/30 space-y-4 animate-in fade-in">
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            <div>
                                                <label className="block text-[10px] font-bold text-zinc-500 uppercase mb-1">Título</label>
                                                <input
                                                    className="w-full bg-[#0f1014] border border-zinc-800 rounded-lg px-3 py-1.5 text-sm text-white focus:border-gold-500 outline-none"
                                            ...
                                            <div>
                                                ...
                                                <input
                                                    type="datetime-local"
                                                    className="w-full bg-[#0f1014] border border-zinc-800 rounded-lg px-3 py-1.5 text-sm text-white focus:border-gold-500 outline-none [color-scheme:dark]"
                                        ...
                                        <div className="flex justify-end gap-3 pt-2 border-t border-zinc-800">
                                            ...
                                            <button
                                                ...
                                                className="px-6 py-2 bg-gold-600 hover:bg-gold-500 text-black rounded-lg text-xs font-bold transition-colors"
                                            >
                                                Salvar Alterações
                                            </button>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-center">
                                        ...
                                        <div className="flex items-center gap-3 group/client">
                                            <div className="w-10 h-10 rounded-full bg-zinc-900 border border-zinc-800 flex items-center justify-center text-xs font-bold text-zinc-500 group-hover/client:border-gold-500/50 group-hover/client:text-gold-500 transition-all">
                                        ...
                                        {/* Event Info */}
                                        <div>
                                            <h4 className="text-[10px] font-bold text-zinc-500 uppercase mb-1 flex items-center gap-1">
                                                <MapPin size={12} /> Local e Hora
                                            </h4>
                                        ...
                                        <div className="flex items-center justify-end gap-2">
                                            ...
                                            <button
                                                ...
                                                className="p-2 text-zinc-400 hover:text-gold-500 hover:bg-gold-500/10 rounded-lg transition-colors relative z-50"
                    ...
                    <div className="flex flex-col items-center justify-center py-20 text-center">
                        ...
                        <h3 className="text-xl font-bold text-zinc-400">Nenhuma perícia encontrada</h3>
                        <p className="text-zinc-500 mt-2 max-w-sm">
                            {filterMode === 'upcoming'
                                ? 'Não há perícias agendadas para o futuro. Que tal agendar uma nova?'
                                : 'Não encontramos registros com os filtros atuais.'}
                        </p>
                        <button onClick={() => setIsNewModalOpen(true)} className="mt-6 text-gold-500 hover:text-gold-400 text-sm font-medium">
                            + Agendar agora
                        </button>
                    </div>
                )}
            ...
            <NewPericiaModal isOpen={isNewModalOpen} onClose={() => setIsNewModalOpen(false)} />
        </div>
    );
};

export default Expertise; // Named export not fully supported by lazy load wrapper in App.tsx without modification, keeping default export.
