...
export const AppProvider = ({ children }: { children?: ReactNode }) => {
        ...
        if (usingDb) {
            try {
                const [
                    ...
                    supabase.from('clients').select('*').order('nome_completo', { ascending: true }),
                ...
                if (notifData) {
                    console.log("🔔 [AppContext] Notifications Fetched:", notifData);
    ...
    const saveGlobalPreferences = async (prefs: UserPreferences) => {
    ...
    const deleteClient = async (id: string, reason?: string) => {
    ...
    const updateCase = async (updatedCase: Case, reason?: string) => {
        // Obter estado anterior para log incremental (Regra 1)
    ...
    const updateOfficeExpense = async (expense: OfficeExpense) => {
    ...
    const toggleOfficeExpenseStatus = async (id: string) => {
    ...
    const deleteCaptador = async (id: string) => {
        const { error } = await supabase.from('captadores').delete().eq('id', id);
        if (error) throw error;
        setCaptadores(prev => prev.filter(c => c.id !== id));
    };

    const createCommissionReceipt = async (receipt: CommissionReceipt, recordIds: string[]) => {
    ...
    const deleteCommissionReceipt = async (id: string) => {
    ...
    const confirmReceiptSignature = async (id: string) => {
    ...
    const uploadReceiptFile = async (id: string, file: File) => {
    ...
    const updateGPS = async (caseId: string, list: GPS[]) => {
    ...
    const addRetirementCalculation = async (calc: any) => {
    ...
    const promoteCalculationToCase = async (calcId: string, caseId: string) => {
    ...
    const triggerRgpSync = async (list: any) => { /* Robot trigger */ };
    const triggerReapSync = async (list: any) => { /* Robot trigger */ };

    // --- NEW FUNCTION: Unified History ---
    const getUnifiedClientHistory = async (clientId: string) => {
    ...
    const contextValue = useMemo(() => ({
        ...
        chats, chatMessages, waitingChatsCount, isAssistantOpen, isStatusBlinking, clients, cases
    ]);
